...
def update_data():
    if e_fname.get() == "" or e_lname.get() == "" or e_email.get() == "" or e_mobile.get() == "" or e_id.get() == "":
        ...
        conn = create_conn()
        cursor = conn.cursor()
        query = "update user set f_name=%s,l_name=%s,email=%s,mobile=%s where id=%s"
...
insert=Button(root,text="INSERT",bg="black",fg="white",font=("Candara",12),command=insert_data)
insert.place(x=20,y=300)

insert=Button(root,text="SEARCH",bg="black",fg="white",font=("Candara",12),command=search_data)
insert.place(x=95,y=300)

insert=Button(root,text="UPDATE",bg="black",fg="white",font=("Candara",12),command=update_data)
insert.place(x=174,y=300)

insert=Button(root,text="DELETE",bg="black",fg="white",font=("Candara",12),command=delete_data)
